...
function(odb_compile outvar)
	...
	foreach(input ${PARAM_FILES})
        ...
        set(ODB_INCLUDE_PREFIX)
        set(dname)
        if(PARAM_RELTO)
            get_filename_component(full_relto "${PARAM_RELTO}" ABSOLUTE)
            get_filename_component(full_input "${input}" ABSOLUTE)
		...
		foreach(sfx ${ODB_COMPILE_FILE_SUFFIX})
			...
			if(NOT "${PARAM_MULTI_DATABASE}" MATCHES "static" OR NOT "${pfx}" MATCHES "common")
				set(output "${ODB_COMPILE_OUTPUT_DIR}/${dname}/${fname}${sfx}${ODB_COMPILE_SOURCE_SUFFIX}")
